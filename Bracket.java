...
public class Bracket implements Serializable {
    ...
    public ArrayList<String> getBracket(){ return bracket; }
    ...
    public void removeAbove(int child){
        if (child==0)
            bracket.set(child,"");
        else {
    ...
    public void resetFullTree(int root) {
    	...
    	if (root ==0){//special behavior to reset final 4
            ...
            if (child1 < 64) { resetFullTree(child1); }//child is above round 1

            if (child2 < 64) { resetFullTree(child2); }
